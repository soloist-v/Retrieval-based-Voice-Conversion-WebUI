...
class SineGen(torch.nn.Module):
    ...
    def __init__(
            ...
            harmonic_num=0,         # 0
            sine_amp=0.1,           # 0.1
            noise_std=0.003,        # 0.003
            voiced_threshold=0,     # 0
            flag_for_pulse=False,   # False
    ):
    ...
    def forward(self, f0: torch.Tensor, upp: int = 480):
...
class SourceModuleHnNSF(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, upp: int = 480):
        # if self.ddtype ==-1:
...
class GeneratorNSF(torch.nn.Module):
    def __init__(
        ...
        self.f0_upsamp = torch.nn.Upsample(scale_factor=math.prod(upsample_rates)) # scale_factor = 480
